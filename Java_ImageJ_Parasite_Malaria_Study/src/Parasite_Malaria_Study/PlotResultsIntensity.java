 ...
 */
public class PlotResultsIntensity implements PlugIn {
	...
	public void run(String arg) {
		 ...
		 */
		GenericDialog gd = new GenericDialog("1.0 Plot Results Intensity");
		
		gd.setEchoChar('*');
		gd.addStringField("Token for MalariaParasitesPlugins", "");
		
		
		gd.addMessage("This plugin displays intensity from normalized files in z");
		gd.showDialog();
		if (gd.wasCanceled())
			return;
		...
		gdproj.addStringField("Prefix to search for ", "ParasitesNormalized_");
							...
							if (attachmenthere)
							{
								...
								float[] blues=new float[32];
								 ...
								 for (int i=0;i<nparasites;i++)
								 {
									 ...
									 float normalizedIntensity=(float) ((this.MyParasites[(5*i)+4]-minintensity)/(maxintensity-minintensity));
									//float normalizedIntensity=(float) (this.MyParasites[(5*i)+4]/(maxintensity));
									 //float normalizedIntensity=(float) (this.MyParasites[(5*i)+4]);
...
{
	int count = 0;
	this.MyParasites = new Float[6 * 1000 ]; // For the moment max number of lines= 500
	int nbParasites=0;
	try 
	{
		if (file.canRead()) 
		{  
	    	...
	    	while (fileScan.hasNext()) 
	    	{
	    	    ...
	    	    this.MyParasites[count] = Float.parseFloat(fileScan.next()); //x
	    	        count++;
	    	    this.MyParasites[count] = Float.parseFloat(fileScan.next()); //y
	    	        count++;   
	    	    this.MyParasites[count] = Float.parseFloat(fileScan.next()); //z normalized
	    	    count++;  
	    	    fileScan.next(); // tpi
	    	    this.MyParasites[count] = Float.parseFloat(fileScan.next()); //intensity
	    	        count++;   
	    	        this.MyParasites[count] =this.MyParasites[count-1]; //intensity again
	    	        count++;  
	    	        fileScan.next(); //dist
	    	        fileScan.next(); // score
